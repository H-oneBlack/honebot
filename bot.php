...
switch ($tlg->Text ()){
	...
	$tlg->sendMessage ([
		'chat_id' => $tlg->ChatID (),
		'text' => 'Bot original @Honeblack por @Honeblack'
	]);
	...
	if ($redis->dbSize () == $limite){
		...
		$senha=mt_rand(111111111111111, 999999999999999);

		exec ('./gerarusuario.sh '.$usuario.' '.$senha.' 1 1');

		$textoSSH="🇧🇷 Conta SSH criada ;)\r\n\r\n<b>Servidor:</b> <code>".Oculto."</code>\r\n<b>Usuario:</b> <code>".$usuario."</code>\r\n<b>Senha:</b> <code>".$senha."</code>\r\n<b>Logins:</b> 1\r\n<b>Validade:</b> ".date ('d/m/yyyy', strtotime('+30 day'))."\r\n\r\n🤙 Senha Com @Honeblack";
